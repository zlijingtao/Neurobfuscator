...
# victim_model = custom_cnn_5(3072)
checkpoint = torch.load("./best_checkpoint_model0_seed{}.tar".format(args.victim_seed))
# checkpoint = torch.load("./best_checkpoint_newvgg_model9_seed{}.tar".format(random_seed))
# checkpoint = torch.load("./best_checkpoint_res_model0_seed{}.tar".format(random_seed))
best_prec1 = checkpoint['best_prec1']
victim_model.load_state_dict(checkpoint['state_dict'])
print("Validation for vgg-11 model is {}".format(best_prec1))
# print("Validation for resnet-20 model is {}".format(best_prec1))
...
class Attack():
    ...
    def ensemble_attack1(self, model_name, target, org_target, logger):
        ...
        #for i in range(2):
            ...
            fake_image = fake_image - grad * self.e # self.e is epsilon in FGSM:  https://arxiv.org/pdf/1706.06083.pdf
            ...
            # if fake_label.item() != fake_pred.item():
            ...
            if (i + 1) % 20 == 0:
                print(str(i) + '\torg prediction: ' + str(org_pred.item()) + '\tfake prediction: ' + str(fake_pred.item()) +
                    '\tattack_pred: ' + str(attack_pred_list) + '\te: ' + str(self.e) + '\tsucc: ' + str(self.succ))
        ...
        str_log = 'src: ' + str(org_target) + '\ttar: ' + str(target)+ '\ttotal: ' + str(i + 1) + '\tsuccess: ' + str(self.succ) + '\tdiff_suc: ' + str(diff_succ) + '\tdif_total: ' + str(diff_all) 
        # print(str_log)
        logger.debug(str_log)
        str_filename = './attack_result/' + model_name + 'seed{}_FGSMnotarget.log'.format(random_seed)
...
def main():
    ...
    model_log_file = "./attack_result/FGSM_Notarget_attack_newvgg11_seed{}.log".format(random_seed)
