...
def model_name_from_seed_depth(seed):
    np.random.seed(seed)
    min_conv_layer = 30
    max_conv_layer = 48
    min_conv_degree = 3 #Max allowed channel is 2^6
    max_conv_degree = 6 #Max allowed channel is 2^6
    min_conv_size = 1 #Max allowed channel is 2^6
    max_conv_size = 80 #Max allowed channel is 2^6
    expansion = 4
    ...
    for i in range(num_conv):
        if i == 0:
            #first conv layer use normal
            conv_choice = 0
        else:
            #[depth:1]
            #100% chance get depthblock
            conv_choice = 1
        if conv_choice == 0:
            #gen_normal_conv2d
            model_name += "_"
            ...
            model_name += "_"
            # Channel Size
            channel_size = 16*np.random.randint(low = min_conv_size, high = max_conv_size)
            model_name += "depth{}p".format(channel_size)
            # Stride Size
            if_pool = int(np.random.binomial(size=(1,), n=1, p = 0.2)[0])
            if if_pool:
    ...
    #     model_name += "_"
    #     # Neuron Size
    #     model_name += "{}".format(160*np.random.randint(low = 1, high = 12))
...
def model_name_from_seed(seed):
    ...
    max_conv_layer = 18
    max_fc_layer = 3
    min_conv_degree = 6 #Max allowed channel is 2^6
    max_conv_degree = 11 #Max allowed channel is 2^6
    min_fc_degree = 6 #Max allowed neuron is 2^10
    max_fc_degree = 11 #Max allowed neuron is 2^10
    max_num_pool = 5 #Set to 5 does not work for mnist (<=2)!
    num_conv = np.random.randint(low = min_conv_layer, high = max_conv_layer)
...
def fc_list_gen(size_fc, transition_size, fc_bn):
    fc_list = []
    if len(size_fc) == 1:
        fc_list.append(["self.classifier", " = torch.nn.Linear({}, {})".format(transition_size, size_fc[0])])
        if fc_bn:
            fc_list.append(["self.classifier_bn", " = torch.nn.BatchNorm1d({}, affine=False)".format(size_fc[0])])
    else:
...
def conv_list_gen(size_conv, filter_conv, stride_conv, padding_conv, input_channel, conv_bn, bias = True):
    conv_list = []
    for i in range(len(size_conv)):
        if i == 0:
            if bias:
                ...
                conv_list.append(["self.conv{}".format(i), " = torch.nn.Conv2d({}, {}".format(input_channel, size_conv[i]) + ", ({}, {}), ".format(filter_conv[i], filter_conv[i]) + "stride=({}, {}), ".format(stride_conv[i], stride_conv[i]) + "padding=({}, {}), bias=False)".format(padding_conv[i], padding_conv[i])])
...
def save_label_from_string(dir_name, info_list, model_name):
    label_fname = dir_name + "depth_batch_{}_nclass_{}_infeat_{}_seed_{}.npy".format(info_list[0], info_list[1], info_list[2], info_list[3])
    mstring_fname = dir_name + "depth_batch_{}_nclass_{}_infeat_{}_seed_{}.mstring".format(info_list[0], info_list[1], info_list[2], info_list[3])
...
def main():
    ...
    parser.add_argument("--input_features", type=int, default=150528, help="flattened input size")
    parser.add_argument("--input_channel", type=int, default=3, help="input channel")
    parser.add_argument("--batch_size", type=int, default=1, help="batch_size")
    parser.add_argument("--num_classes", type=int, default=1000, help="number of classes")
    parser.add_argument("--random_seed", type=int, default=0, help="random seed to generate the model")
    parser.add_argument("--file_name", type=str, default="torch_tvm_prof", help="this script directly modifies torch_tvm_prof.py")
    parser.add_argument("--output_file", type=str, default="depth_batch_1_nclass_1000_infeat_150528_seed_0.csv", help="this script directly modifies torch_tvm_prof.py")
    parser.add_argument("--opt_level", type=int, default="3", help="opt_level set to 3")
    args = parser.parse_args()
    '''format: $0p$1p$2p$3p$4'''
    dir_name = "./trace/"

    model_name = model_name_from_seed_depth(args.random_seed)
    
    # for test only
    # model_name = 'cnn_64p3p1p1p0_256p3p2p1p0_1024p3p1p1p3_depth256p1_256p3p1p1p2_256p3p1p1p0_1024p3p2p1p0_64p3p1p1p0_256p3p1p1p0_res256p1_512p3p1p1p0_res512p1_256p3p1p1p0_bn0_mlp_512_1024_bn0'
    ...
    if num_conv == 0:
        ...
        last_dim_size = size_conv[-1]
        if "res" in last_dim_size:
            last_dim_size = last_dim_size.replace("res", "")
        elif "th" in last_dim_size:
            last_dim_size = last_dim_size.replace("th", "")
        transition_size = int(last_dim_size)

    conv_list = conv_list_gen(size_conv, filter_conv, stride_conv, padding_conv, input_channel, conv_bn, bias = False)
