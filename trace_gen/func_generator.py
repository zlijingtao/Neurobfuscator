...
def model_name_from_seed(seed):
    ...
    max_fc_degree = 13 #Max allowed neuron is 2^10
    max_num_pool = 2
    ...
    for i in range(num_conv):
        model_name += "_"
        # Channel Size
        model_name += "{}p".format(2**np.random.randint(low = min_conv_degree, high = max_conv_degree))
        # Filter Size
        ...
        model_name += "{}p".format(1)
        # Pooling or not
        if_pool = np.random.randint(2)
        pool_counter += if_pool
...
def save_label_from_string(dir_name, info_list, model_name):
    label_fname = dir_name + "batch_{}_nclass_{}_infeat_{}_seed_{}.npy".format(info_list[0], info_list[1], info_list[2], info_list[3])
    mstring_fname = dir_name + "batch_{}_nclass_{}_infeat_{}_seed_{}.mstring".format(info_list[0], info_list[1], info_list[2], info_list[3])
...
def main():
    ...
    if gen_label:
        mstring_path = "./trace/batch_{}_nclass_{}_infeat_{}_seed_{}.mstring".format(args.batch_size, args.num_classes, args.input_features, args.random_seed)
    ...
    # model_name = model_name_from_seed(args.random_seed)
    # for test only
    # model_name = 'cnn_32p3p1p1p0_32p3p1p1p1_64p3p1p1p0_64p3p1p1p1_bn1_mlp_256_256_256_bn1'
