...
#indexDict = {'conv':9, 'fc':10, 'pooling':0, 'pointConv':3, 'relu':5, 'concact':25, 'add':20}
#layer_int_to_name_map = {9:'conv', 10:'fc', 0:'pooling', 3:'pointConv', 5:'relu', 25:'concact', 20:'add'}
#
...
def convert_inputs_to_ctc_format(sampleName, target_layers):
    ...
    print("feature std is")
    feature_mean = np.mean(train_inputs)
    print(np.std(train_inputs))
    print("feature mean is")
    feature_std = np.std(train_inputs)
    print(np.mean(train_inputs))
