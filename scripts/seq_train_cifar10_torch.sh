...
for val1 in ${num_hidden_list[*]}; do
    for val2 in ${dataset_type_list[*]}; do
        num_hidden=$val1
        dataset_type=$val2
        model_name=deepsniffer_LSTM_both_autotvm_${normalize}_${num_hidden}_cifar10_torch_infer

        if [[ "${dataset_type}" == "reduced" ]]; then
            model_path="./${choice}/predictor/logs_${model_name}"
            dataset="./${choice}/dataset/cifar10_train_data_torch_infer_dict.pickle"
        elif [[ "${dataset_type}" == "full" ]]; then
            model_path="./${choice}/predictor/logs_full_${model_name}"
            dataset="./${choice}/dataset/cifar10_train_data_torch_infer_dict_full.pickle"
